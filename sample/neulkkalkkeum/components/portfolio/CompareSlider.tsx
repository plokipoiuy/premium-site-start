...
export function CompareSlider({ beforeAlt, afterAlt, beforeImage, afterImage, className = "" }: CompareSliderProps) {
  ...
  return (
    ...
    >
      ...
      <span className="pointer-events-none absolute right-4 top-4 z-10 rounded-full bg-neul-green/85 px-3 py-1 text-xs font-bold uppercase tracking-wide text-white">
        After
      </span>
      ...
      <div
        ...
        onKeyDown={onKeyDown}
        className="absolute top-0 h-full w-1 cursor-ew-resize bg-white shadow-[0_0_0_1px_rgba(0,0,0,0.05)] focus-visible:outline-neul-green"
        style={{ left: `calc(${position}% - 2px)` }}
      >
        <div className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white text-neul-green shadow-card">
