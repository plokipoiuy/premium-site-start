...
import { sampleReviews } from "@/sample/neulkkalkkeum/lib/data/testimonials";
    ...
    <section id="testimonials" className="section-padding bg-white">
      <div className="container-content">
        <Reveal className="mx-auto max-w-2xl text-center">
          <span className="font-manrope text-xs font-semibold uppercase tracking-[0.2em] text-neul-pink">
            REVIEWS
          </span>
          ...
          {sampleReviews.map((review, i) => (
            <Reveal key={review.id} delay={i * 0.1}>
              <div className="relative flex h-full flex-col items-center gap-4 rounded-card border border-divider bg-neul-bg p-8 text-center">
                ...
                <Quote size={22} className="mt-2 text-neul-sage" aria-hidden="true" />
                <div className="flex gap-0.5">
                  {Array.from({ length: review.rating }).map((_, starIndex) => (
                    <Star key={starIndex} size={14} className="fill-neul-pink text-neul-pink" aria-hidden="true" />
                  ))}
                </div>
