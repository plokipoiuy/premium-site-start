...
import { checklistItems } from "@/sample/neulkkalkkeum/lib/data/checklist";
import { partnerCompanies } from "@/sample/neulkkalkkeum/lib/data/partners";
              ...
              <ul className="mt-5 flex flex-1 flex-col gap-4">
                {checklistItems.map((item) => (
                  <li key={item.id} className="flex items-start gap-3">
                    <CheckCircle2 size={20} className="mt-0.5 shrink-0 text-neul-green" aria-hidden="true" />
          ...
          <Reveal delay={0.1} className="h-full">
            <div className="flex h-full flex-col rounded-card bg-white p-6 shadow-soft lg:p-8">
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="rounded-full bg-neul-pink-light px-2.5 py-1 text-xs font-bold text-neul-pink">
                  샘플
                </span>
              </div>
