...
import { services } from "@/sample/neulkkalkkeum/lib/data/services";
...
export function ServicesSection() {
  return (
    <section id="services" className="section-padding bg-neul-pink-light/60">
            ...
            <Reveal key={service.id} delay={i * 0.1} className="h-full">
              <article className="group flex h-full flex-col overflow-hidden rounded-card bg-white shadow-soft">
                ...
                <div className="flex flex-1 flex-col gap-3 p-6">
                  <div>
                    <span className="font-manrope text-[11px] font-semibold uppercase tracking-[0.18em] text-neul-pink">
                  ...
                  <ul className="mt-auto flex flex-col gap-1.5">
                    {service.scope.slice(0, 3).map((item) => (
                      <li key={item} className="flex items-start gap-2 text-xs text-text-muted">
                        <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-neul-sage" aria-hidden="true" />
                        {item}
                      </li>
                    ))}
                  </ul>
