...
import { featuredCase, portfolioCases } from "@/sample/gridaclean/lib/data/portfolio";
...
import { PlaceholderImage } from "@/sample/gridaclean/components/ui/PlaceholderImage";
import { CompareSlider } from "@/sample/gridaclean/components/portfolio/CompareSlider";
import { Modal } from "@/sample/gridaclean/components/ui/Modal";
...
function InfoRow({ icon: Icon, label }: { icon: typeof MapPin; label: string }) {
  return (
    <div className="flex items-center gap-2 text-sm text-text-body">
      <Icon size={16} className="shrink-0 text-grida-sky" aria-hidden="true" />
      {label}
    </div>
  );
}
        ...
        <div className="mt-10 flex gap-2 overflow-x-auto scrollbar-thin pb-1 lg:mt-12 lg:flex-wrap lg:justify-center">
          {categories.map((cat) => (
            <button
              ...
              className={`shrink-0 rounded-full px-5 py-2.5 text-sm font-semibold transition-colors focus-visible:outline-grida-blue ${
                category === cat
                  ? "bg-grida-blue text-white"
                  : "bg-white text-text-body hover:bg-grida-blue/8"
              }`}
            >
              {cat}
            </button>
        ...
        <Reveal className="mt-10 lg:mt-14">
          <div className="grid gap-8 rounded-card bg-white p-5 shadow-card lg:grid-cols-[1.3fr_1fr] lg:gap-10 lg:p-8">
            ...
            <div className="flex flex-col justify-center gap-5">
              <div>
                <span className="rounded-full bg-grida-accent-light px-3 py-1 text-xs font-bold text-grida-accent">
                  대표 사례 · 샘플 데이터
                </span>
                ...
                <ul className="flex flex-col gap-1.5">
                  {featuredCase.scope.map((s) => (
                    <li key={s} className="flex items-start gap-2 text-sm text-text-body">
                      <span className="mt-1.5 h-1 w-1 shrink-0 rounded-full bg-grida-blue" aria-hidden="true" />
                      {s}
                    </li>
                  ))}
                </ul>
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 overflow-x-hidden sm:grid-cols-2 lg:mt-12 lg:grid-cols-3 lg:gap-6">
          {filtered.map((item, i) => (
            <motion.button
              ...
              className="group overflow-hidden rounded-card bg-white text-left shadow-soft transition-shadow hover:shadow-card focus-visible:outline-grida-blue"
            >
              ...
              <div className="p-5">
                <span className="text-xs font-semibold uppercase tracking-wide text-grida-accent">
                  {item.category}
                </span>
