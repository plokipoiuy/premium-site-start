...
import { sampleReviews } from "@/sample/gridaclean/lib/data/testimonials";
    ...
    <section id="testimonials" className="section-padding bg-white">
      <div className="container-content">
        <Reveal className="mx-auto max-w-2xl text-center">
          <span className="font-manrope text-xs font-semibold uppercase tracking-[0.2em] text-grida-accent">
            REVIEWS
          </span>
          ...
          {sampleReviews.map((review, i) => (
            <Reveal key={review.id} delay={i * 0.1}>
              <div className="relative flex h-full flex-col items-center gap-4 rounded-card border border-divider bg-ivory p-8 text-center">
                <span className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-grida-accent px-3 py-1 text-xs font-bold text-white">
                  샘플
                </span>
                <Quote size={22} className="mt-2 text-grida-sky" aria-hidden="true" />
                <div className="flex gap-0.5">
                  {Array.from({ length: review.rating }).map((_, starIndex) => (
                    <Star key={starIndex} size={14} className="fill-grida-accent text-grida-accent" aria-hidden="true" />
                  ))}
                </div>
