...
import { chatbotOptions } from "@/sample/gridaclean/lib/data/chatbot";
    ...
    <div className="fixed bottom-24 right-4 z-40 flex flex-col items-end gap-3 lg:bottom-8 lg:right-8">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="flex items-center justify-between bg-grida-blue px-5 py-4">
              ...
              {!selected ? (
                  ...
                  <div className="flex flex-col gap-2">
                    {chatbotOptions.map((option) => (
                      <button
                        ...
                        className="rounded-xl border border-divider px-4 py-3 text-left text-sm font-medium text-text-charcoal transition-colors hover:border-grida-blue hover:bg-grida-blue/5 focus-visible:outline-grida-blue"
                      >
                        {option.label}
                      </button>
                    ))}
                ...
                <>
                  <button
                    ...
                    className="flex items-center gap-1.5 self-start text-xs font-semibold text-text-muted transition-colors hover:text-grida-blue focus-visible:outline-grida-blue"
                  >
                  ...
                  <p className="rounded-xl bg-grida-blue/5 px-4 py-3 text-sm leading-[1.7] text-text-body">
                    {selected.reply}
                  </p>
                </>
              )}
      ...
      <AnimatePresence>
        {showTooltip && !isOpen && (
          <motion.button
            ...
            onClick={openChat}
            className="rounded-2xl rounded-br-sm bg-white px-4 py-2.5 text-sm font-medium text-text-charcoal shadow-card focus-visible:outline-grida-blue"
          >
            청소 견적이 궁금하신가요?
          </motion.button>
        )}
      ...
      <button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-grida-blue text-white shadow-card transition-transform hover:scale-105 active:scale-95 focus-visible:outline-grida-blue"
      >
