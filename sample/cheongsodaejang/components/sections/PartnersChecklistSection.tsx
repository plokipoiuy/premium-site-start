...
import { checklistItems } from "@/sample/cheongsodaejang/lib/data/checklist";
import { partnerCompanies } from "@/sample/cheongsodaejang/lib/data/partners";
              ...
              <ul className="mt-5 flex flex-1 flex-col gap-4">
                {checklistItems.map((item) => (
                  <li key={item.id} className="flex items-start gap-3">
                    <CheckCircle2 size={20} className="mt-0.5 shrink-0 text-cheongso-blue" aria-hidden="true" />
          ...
          <Reveal delay={0.1} className="order-first h-full lg:order-none">
            <div className="flex h-full flex-col rounded-card bg-white p-6 shadow-soft lg:p-8">
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="rounded-full bg-cheongso-accent-light px-2.5 py-1 text-xs font-bold text-cheongso-accent">
                  샘플
                </span>
              </div>
