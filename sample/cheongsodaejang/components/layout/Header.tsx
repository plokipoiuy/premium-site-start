...
import { Logo } from "@/sample/cheongsodaejang/components/ui/Logo";
import { Button } from "@/sample/cheongsodaejang/components/ui/Button";
import { navLinks, quoteCta } from "@/sample/cheongsodaejang/lib/data/nav";
        ...
        <nav className="hidden items-center gap-8 lg:flex" aria-label="주요 메뉴">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className={`text-base font-medium transition-colors hover:text-cheongso-accent ${
        ...
        <div className="flex items-center gap-3 lg:hidden">
          ...
          <button
            ...
            className={`flex h-10 w-10 items-center justify-center rounded-full transition-colors ${
              solid ? "text-cheongso-blue hover:bg-cheongso-blue/5" : "text-white hover:bg-white/10"
            }`}
          >
            ...
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  ...
                  className="block rounded-lg px-3 py-3 text-base font-medium text-text-charcoal hover:bg-cheongso-blue/5"
                >
                  {link.label}
                </a>
              </li>
