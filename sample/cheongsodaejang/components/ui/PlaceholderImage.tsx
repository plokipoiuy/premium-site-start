...
const variantStyles: Record<NonNullable<PlaceholderImageProps["variant"]>, string> = {
  sage: "from-cheongso-sky/35 to-cheongso-blue/15",
  blush: "from-cheongso-accent-light to-cheongso-sky/20",
  ivory: "from-cheongso-sky/20 to-ivory",
};
...
export function PlaceholderImage({ alt, className = "", variant = "sage" }: PlaceholderImageProps) {
  return (
    ...
    >
      <ImageIcon className="h-8 w-8 text-cheongso-blue/40" strokeWidth={1.5} aria-hidden="true" />
      <span className="px-4 text-center text-xs font-medium text-cheongso-blue/50">샘플 이미지</span>
    </div>
  );
}
